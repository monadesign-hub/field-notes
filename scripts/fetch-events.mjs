#!/usr/bin/env node
// Fetches upcoming AI events across the SF Bay Area from Luma's public discover feeds
...
// ── Config ──────────────────────────────────────────────────────────────────
// Luma discover places that make up the SF Bay Area. San Francisco is the only
// currently-launched Bay Area hub and already spans the peninsula and South Bay;
// the others are listed so coverage expands automatically if/when Luma launches them.
const PLACES = [
  "discplace-BDj7GNbGlsF7Cka", // San Francisco (Bay Area hub)
  "discplace-try4RBCGnp3pIy1", // San Jose / South Bay
  "discplace-lmtnZ7h4EamoJWt", // Santa Cruz
];
...
// ── Fetch one discover place, paging until we pass the end bound ─────────────
async function fetchPlaceEvents(placeId, end) {
  const out = [];
  let cursor = null;
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    if (lastStart && lastStart > end) break; // feed is chronological; past our window
    cursor = data.next_cursor;
  }
  return out;
}

// ── Fetch every Bay Area place and merge ─────────────────────────────────────
async function fetchBayAreaEvents(end) {
  const all = [];
  for (const placeId of PLACES) {
    try {
      const part = await fetchPlaceEvents(placeId, end);
      console.log(`[fetch-events]   ${placeId}: ${part.length} events`);
      all.push(...part);
    } catch (err) {
      console.error(`[fetch-events]   ${placeId} failed: ${err.message}`);
    }
  }
  return all;
}
...
function shape(entry) {
  ...
  const rawVenue = isVirtual ? "Virtual" : (geo.city_state || geo.city || "San Francisco, CA");
  const venue = rawVenue
    .replace(/,\s*California$/i, ", CA")
    .replace(/^San Francisco(,\s*CA)?$/i, "SF");
  return {
...
function renderItem(e) {
  ...
  const meta = [esc(e.venue), e.host && esc(e.host)].filter(Boolean).join(" · ");
  return `      <a class="up-item" href="${esc(e.url)}" target="_blank" rel="noopener noreferrer">
        <div class="up-time">${esc(time)}</div>
        <div class="up-body">
          <span class="up-title">${esc(e.name)}${e.isFree ? '<span class="up-free">Free</span>' : ""}</span><span class="up-sub">${meta}</span>
        </div>
...
async function main() {
  ...
  const raw = await fetchBayAreaEvents(end);
  console.log(`[fetch-events] fetched ${raw.length} Bay Area events from Luma`);
